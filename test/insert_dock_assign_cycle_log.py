            ...
            for item in items:
                if item.get('dock_assign_time', False) and item.get('dock_no', False):
                    ...
                    if dock_no is not None:
                        if dock_no[0] == int(item['dock_no']):
                            if item['dock_assign_time'] == dock_no['dock_assign_time']:
                                continue
                            else:
                                self.cursor.execute(f"DELETE FROM truck_cycle where timestamp='{item['timestamp']}' and truck_no='{item['vehicle_number']}'")
                                self.connection.commit()
                                self.logger.info(f"Deleted old entry for truck_no '{item['vehicle_number']}' with timestamp '{item['timestamp']}'")
