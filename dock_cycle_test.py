...
#current_data = '10-04-2025'

DYNAMODB_TABLE = 'rhenus_dock_cycle_testing'
...
while True:
    ...
    last_cycle_query = "SELECT * FROM dock_cycle_testing WHERE dock='dock{}' ORDER BY timestamp DESC FETCH FIRST ROW ONLY"
    for dock_no in range(1, 21):
        ...
        for data in dock_data:
            if data['state_change'] == 1 and last_dock_cycle_status == -1:
                ...
                # Insert into DynamoDB and Postgres immediately
                ...
                cursor.execute(f"""
                    INSERT INTO dock_cycle_testing (timestamp, arrival, status, dock) 
                ...
                # Check next 2 dock_status values after this timestamp
                cursor.execute(f"""
                    SELECT dock_status FROM time_bucket_dock{dock_no}
                    WHERE timestamp > '{data['next_timestamp']}'
                    ORDER BY timestamp
                    LIMIT 2
                """)
                next_statuses = [row[0] for row in cursor.fetchall()]

                # If next 2 statuses are 3 → false dock_in, revert
                if len(next_statuses) == 2 and all(int(s) == 2 for s in next_statuses):
                    print("⚠️ False dock_in detected. Reverting entry...")

                    # Delete from Postgres
                    cursor.execute(f"""
                        DELETE FROM dock_cycle_testing 
                        WHERE timestamp = '{upload_timestamp}' AND dock = 'dock{dock_no}'
                    """)
                    conn.commit()
                ...
                if updated_row:
                    updated_row_timestamp = updated_row['timestamp']

                    print(f"Dock_no: dock{dock_no}, Departure: {departure_time}")

                    update_dynamodb({
                        'timestamp': str(updated_row_timestamp).split('.')[0].replace(' ', 'T'),
                        'arrival': str(arrival_time).split('.')[0].replace(' ', 'T'),
