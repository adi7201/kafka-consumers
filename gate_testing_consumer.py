...
class KafkaJsonConsumer:
    ...
    def parsing(self, data):
        """
        Parse and validate Indian number plate format
        Returns the parsed plate number or original data if validation fails
        """
        original_data = "".join(data).upper().replace(' ', '').replace('IND', '')
        data = original_data
        ...
        # Check length - if invalid, return original data
        if len(data) < 8 or len(data) > 12:
            return original_data
        
        state = state.replace('AH', 'MH').replace('NH', 'MH').replace('HH', 'MH').replace('WH', 'MH').replace('RO', 'RJ').replace('DO', 'DD').replace('OO', 'DD').replace('HX', 'HR')
        state_code = data[2:4].replace('A', '4').replace('O', '0').replace('Q', '0').replace('S', '5').replace('Z', '4').replace('T', '1').replace('D', '0')
        number = data[-4:].replace('A', '4').replace('O', '0').replace('Q', '0').replace('S', '5').replace('B', '8')
        rest = data[4:-4]

        indian_states_and_ut_codes = [
            "AP", "AR", "AS", "BR", "CG", "GA", "GJ", "HR", "HP", "JH", "KA", "KL", "MP", "MH",
            "MN", "ML", "MZ", "NL", "OR", "PB", "RJ", "SK", "TN", "TG", "TR", "UP", "UK", "WB",
            "AN", "CH", "DN", "LD", "DL", "PY", "JK", "LA", "BH", "DD"
        ]
        
        # Check state code - if invalid, return original data
        if state not in indian_states_and_ut_codes:
            return original_data
        
        return state + state_code + rest + number       
    ...
    def insert_to_postgres(self, message):
        try:
            ...
            with self.db_connection.cursor() as cursor:
                ...
                # SQL insert query
                insert_query = """
                INSERT INTO rhenus_events (timestamp, vehicle_number, event_source, active, filename, vehicle_type, gate_id, unique_id, event_json)
